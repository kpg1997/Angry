...
import { ethgetsaga, ethgetchart } from '../modules/ethchart';
...
function Ethereum() {
    ...
    const gogo = useSelector((state) => state.ethchart.gogo);
    const charting = useSelector((state) => state.ethchart.dataing);

    useEffect(() => {
        dispatch(ethgetchart());
        dispatch(ethgetsaga());
    }, [ethgetsaga, ethgetchart]);

    const data = useSelector((state) => state.ethchart.data);
    ...
    return (
        <div style={{ width: "100%", background: "none",height:"90%" }}>
            ...
            <div style={{ width: "35%", background: "rgb(243,240,237)", float: "left", marginTop: "20px", borderRadius:"15px",padding:"10px",boxShadow:"0 0 3px 3px rgb(180,180,180)",marginLeft:"3px" }}>
                ...
                <div style={{width:"100%",float:"left",borderBottom:"1px solid rgb(200,200,200)"}}><h2><b>이더리움</b></h2></div>
        ...
        // <div>
        //     <div>현재가:{charting[0]}</div>
        //     {charting[1] == 'RISE' ? (
        //         <div>
        //             전일대비 +{charting[3]}% ▲ {charting[2]}
        //         </div>
        //     ) : (
        //         <div>
        //             전일대비 -{charting[3]}% ▼ {charting[2]}
